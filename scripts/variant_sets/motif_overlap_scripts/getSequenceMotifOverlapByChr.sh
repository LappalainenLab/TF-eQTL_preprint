...
line=`head -n $1 $hocomoco_file | tail -n 1`
...
out_file=motif_overlap_vars/$TF.chr$chrom.`basename $hocomoco_file | sed 's/tsv//'`$qual.onlyuppermotif.`basename $input_vcf | sed 's/vcf.gz//'`txt


echo "Testing TF $TF motif $motif, quality $qual, for overlap with $input_vcf, chr$chrom"
...
gunzip < $input_vcf | grep -P "^chr$chrom\t" | while read line
do
